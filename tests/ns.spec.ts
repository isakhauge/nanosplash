...
describe('useNs', () => {
    ...
    describe('show', () => {
        ...
        it('inserts the Nanosplash before existing children so it renders on top', () => {
            ...
            expect(container.firstElementChild?.classList.contains(ClassNames.ns)).toBe(true)
    ...
    describe('timing', () => {
        ...
        it('sets the show delay custom property on the host', () => {
            ...
            expect(document.body.style.getPropertyValue('--ns-show-delay')).toBe('150ms')
        })

        it('defaults the show delay to 0ms without timing options', () => {
            ns.show('Now')
            expect(document.body.style.getPropertyValue('--ns-show-delay')).toBe('0ms')
        })
    ...
    describe('job pairs', () => {
        ...
        it('hides and rethrows when the job rejects', async () => {
            const failing = ns.show(['Working…', () => Promise.reject(new Error('boom'))])
        ...
        it('runs an array of pairs sequentially, updating the label per job', async () => {
            ...
            const results = await ns.show([
                ['Step one', async () => { labels.push(currentLabel()); return 1 }],
                ['Step two', async () => { labels.push(currentLabel()); return 'two' }],
                ['Step three', async () => { labels.push(currentLabel()); return true }],
            ])
        ...
        it('reuses a single splash element across the sequence', async () => {
            ...
            await ns.show([
                ['A', async () => { firstElement = get(Selectors.ns) }],
                ['B', async () => { secondElement = get(Selectors.ns) }],
            ])
        ...
        it('fails fast: later jobs never start after a rejection', async () => {
            ...
            const failing = ns.show([
                ['One', async () => { ran.push('one'); return 1 }],
                ['Two', async () => { ran.push('two'); throw new Error('boom') }],
                ['Three', async () => { ran.push('three'); return 3 }],
            ])
        ...
        it('shows spinner only when a pair has empty text', async () => {
            ...
            await ns.show([null, async () => { seenText.push(textVisible()) }])
            await ns.show(['', async () => { seenText.push(textVisible()) }])
            await ns.show([undefined, async () => { seenText.push(textVisible()) }])
        ...
        it('drops the text mid-sequence when a later pair has empty text', async () => {
            ...
            await ns.show([
                ['Step one', async () => { states.push(textVisible()) }],
                [null, async () => { states.push(textVisible()) }],
            ])
