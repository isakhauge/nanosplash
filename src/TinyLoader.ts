...
import {appendFirst, display, fitParentDimensions, get, mk, move, setAttribute} from "./util/dom";
...
export class TinyLoader {
    ...
    private static readonly DEFAULT_BACKGROUND_COLOR: string = 'rgba(255, 255, 255, 0.75)'
    private static readonly DEFAULT_BACKGROUND_BLUR: boolean = true

    public constructor() {
        this.defaultText = TinyLoader.DEFAULT_TEXT
        this.defaultDestination = TinyLoader.DEFAULT_DESTINATION

        // Build UI elements
        this.mainElement = TinyLoader.makeMainElement()
        this.splashElement = TinyLoader.makeSplashElement()
        this.textElement = TinyLoader.makeTextElement()

        // Assemble UI elements
        TinyLoader.assembleElements(this.mainElement, this.splashElement, this.textElement)

        // Insert loader into destination element
        appendFirst(TinyLoader.DEFAULT_DESTINATION, this.mainElement)
     ...
     * @param {Config} config
     */
    public configure(config: Config): TinyLoader {
        if (config?.default?.destination) {
            this.defaultDestination = TinyLoader.getDestinationElement(config.default.destination)
        }
    ...
    private setDefaultStyles(): void {
        this.setTextFontFamily(TinyLoader.DEFAULT_TEXT_FONT)
        this.setTextColor(TinyLoader.DEFAULT_TEXT_COLOR)
        this.setTextSize(TinyLoader.DEFAULT_TEXT_SIZE)
        this.setSplashWidth(TinyLoader.DEFAULT_SPLASH_WIDTH)
        this.setSplashAnimation(TinyLoader.DEFAULT_SPLASH_ANIMATION)
        this.setBackgroundColor(TinyLoader.DEFAULT_BACKGROUND_COLOR)
        this.setBackgroundBlur(TinyLoader.DEFAULT_BACKGROUND_BLUR)
    }
    ...
    public show(text?: string): DestinationController {
        this.setText(text ?? TinyLoader.DEFAULT_TEXT)
    ...
    private moveTo(destination: Destination): void {
        const destinationNode = TinyLoader.getDestinationElement(destination)
    ...
    private static makeMainElement(): HTMLDivElement {
        const mainElement = mk('div', {
            id: 'tiny-loader',
            attributes: [
                {key: 'data-blur', value: String(TinyLoader.DEFAULT_BACKGROUND_BLUR)},
                {key: 'data-splash-animation', value: TinyLoader.DEFAULT_SPLASH_ANIMATION}
            ]
        }) as HTMLDivElement
        mainElement.style.backgroundColor = TinyLoader.DEFAULT_BACKGROUND_COLOR
        return mainElement
    }
    ...
    private static makeSplashElement(): HTMLImageElement {
        const splashElement = mk('img', {
            className: 'tl-splash',
            attributes: [
                {key: 'src', value: TinyLoader.DEFAULT_SPLASH_SOURCE},
                {key: 'alt', value: 'TinyLoader indicator'}
            ]
        }) as HTMLImageElement
    ...
    private static makeTextElement(): HTMLDivElement {
        return mk('div', {
            className: 'tl-text',
            content: TinyLoader.DEFAULT_TEXT
        }) as HTMLDivElement
    }
    ...
    private static getDestinationElement(destination: Destination): Element {
        ...
        if (isString) {
            ...
            if (TinyLoader.isElementOrNode(destinationObject)) {
                return destinationObject as Element
            }
            throw new InvalidDestinationException('The DOM selector does not point to an Element')
        } else if (TinyLoader.isFunction(destination)) {
            const destinationObject = (destination as Function)()
            if (TinyLoader.isElementOrNode(destinationObject)) {
                return destinationObject
            }
            throw new InvalidDestinationException('The destination callback returned an invalid value')
        } else if (TinyLoader.isElementOrNode(destination)) {
     ...
     *
     * @param {TinyLoader} instance
     * @private
     * @static
     */
    public static injectInstanceIntoGlobalScope(instance: TinyLoader): void {
